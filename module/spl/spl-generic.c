...
{
	//mutex_unlock(&mctxt_mutex);
	//mutex_unlock(mctxt_mutex);
	//mutex_exit(&mctxt_mutex);
	//printk(KERN_INFO "mctxt: Device closed\n");

	return 0;
}

static int mctxt_open(struct inode *inodep, struct file *filep)
{
	int ret = 0;

	//if(!mutex_trylock(&mctxt_mutex)) {
	/*
	if(!mutex_enter(&mctxt_mutex)) {	
		pr_alert("mctxt: device busy!\n");
		ret = -EBUSY;
		goto out;
	}
*/
	//mutex_enter(&mctxt_mutex);
	//printk(KERN_INFO "mctxt: Device opened\n");
