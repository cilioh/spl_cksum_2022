...
int get_random_id(size_t bytes)
{
	...
	if (rand < 0)
		return (rand);
	while (NULL != filp) {
		ssize_t rc = file_read(filp, rand, bytes);
		if (rc < 0)
			break;
	}
	...
	return charToInt(rand);
}

EXPORT_SYMBOL(charToInt);
    ...
    taskq_ent_t *t)
{
	...
	//JW_chk
	//if(t->tqent_zio == 1){
		//printk(KERN_WARNING "SPL-ORI: name:%s nactive:%d [%u] \n", tq->tq_name, tq->tq_nactive, t->tqent_zio_id);
		//t->tqent_zio = 0;
	//	dump_stack();
...
taskq_thread(void *args)
{
	...
	while (!kthread_should_stop()) {
		...
		if ((t = taskq_next_ent(tq)) != NULL) {
			...
			//else if (t->tqent_zio == 2){
				//if(xx != t->tqent_zio_id)
				//	printk(KERN_WARNING "SPL-THR-DUP: name:%s nactive:%d [%u] \n", tq->tq_name, tq->tq_nactive, t->tqent_zio_id);
				//list_del_init(&t->tqent_list);
			//}
			/*
			else if (t->tqent_zio == 3){
				printk(KERN_WARNING "THR-DUP+ORI-DUP: name:%s [%u] \n", tq->tq_name, t->tqent_zio_id);
				list_del_init(&t->tqent_list);
				continue;
			}
			*/
			...
			if (t->tqent_flags & TQENT_FLAG_PREALLOC) {
				...
				//JW_chk
				//dup_task.tqent_zio = t->tqent_zio;
				//dup_task.tqent_zio_id = t->tqent_zio_id;
				//dup_task.tqent_lock = t->tqent_lock;
...
//printk(KERN_WARNING "CKSUM_TASKQ_THREAD: name:%s nthreads:%d \n", tq->tq_name, tq->tq_nthreads);
	...
	while (!kthread_should_stop()) {
		if (list_empty(&tq->tq_pend_list) &&
		    list_empty(&tq->tq_prio_list)) {
//printk(KERN_WARNING "BEFORE TASKQ 1111 name:%s\n", tq->tq_name);
		//}
			if (taskq_thread_should_stop(tq, tqt)) {
//printk(KERN_WARNING "WAKING UP 1111\n");
				wake_up_all(&tq->tq_wait_waitq);
				break;
			}
...
//printk(KERN_WARNING "[D]SPL-CHK: name:%s tqent_id:%u nactive:%d [%u] \n", tq->tq_name, t->tqent_id, tq->tq_nactive, t->tqent_zio_id);
			...
			tqt->tqt_flags = 0;
			//--
//printk(KERN_WARNING "WAKING UP 2222\n");
			wake_up_all(&tq->tq_wait_waitq);
		} 
		else {
//printk(KERN_WARNING "BEFORE TASKQ 2222\n");
			if (taskq_thread_should_stop(tq, tqt))
				break;
		}
